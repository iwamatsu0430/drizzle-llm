 ...
 */
export async function validateQueries(_config: DrizzleLLMConfig): Promise<void> {
...
function generateDistributedQueryFile(queries: GeneratedQuery[], sourceFile: string): string {
  ...
  const typeImports = customTypes.length > 0 ? generateTypeImports(customTypes, sourceFile) : "";
...
function findTypeDefinition(sourceFile: any, typeName: string, currentFilePath: string) {
  ...
  for (const importDecl of importDeclarations) {
    ...
    const _typeOnlyImport = importDecl.isTypeOnly();
