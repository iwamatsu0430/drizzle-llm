...
export class OpenAIProvider implements LLMProvider {
   ...
   */
  constructor(apiKey: string, model = "gpt-4", config?: DrizzleLLMConfig) {
...
export class AnthropicProvider implements LLMProvider {
   ...
   * @param config - Optional full configuration for debug settings
   */
  constructor(apiKey: string, model = "claude-3-sonnet-20240229", config?: DrizzleLLMConfig) {
...
export class QueryGenerator {
   ...
   */
  async generateQueriesConversational(
  ...
  private displayBuildSummary(
    ...
    if (results.length > 0) {
      ...
      results.forEach((result, index) => {
        const truncatedIntent =
          result.intent.length > 50 ? `${result.intent.substring(0, 50)}...` : result.intent;
        console.log(`  ${index + 1}. "${truncatedIntent}"`);
        console.log(
          `     └─ Parameters: ${result.parameters.length > 0 ? result.parameters.join(", ") : "none"}`
        );
      });
    }
    ...
    if (failed.length > 0) {
      ...
      failed.forEach((query, index) => {
        const truncatedIntent =
          query.intent.length > 50 ? `${query.intent.substring(0, 50)}...` : query.intent;
        console.log(`  ${index + 1}. "${truncatedIntent}"`);
      });
    }
